...
function buildUploadInventoryText(merchantData: MerchantDataLike): string {
  ...
  return uploads
    .map((upload) => {
      const mode = upload.data?.trim() ? 'content attached' : 'metadata only';
      return `- ${upload.field}: ${upload.name} (${upload.mimeType}, ${mode})`;
    })
    .join('\n');
}

function buildPromptText(merchantData: MerchantDataLike): string {
...
Uploaded Documents:
${buildUploadInventoryText(merchantData)}
...
6. Summarize what the uploaded files appear to contain in documentSummary.
...
async function runUnderwriting(merchantData: MerchantDataLike): Promise<UnderwritingApiResult> {
  ...
  try {
    const content: Array<Record<string, unknown>> = [
      {
        type: 'input_text',
        text: buildPromptText(merchantData),
      },
    ];
